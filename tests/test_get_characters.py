from rest_api_test_case_base import RestApiTestCaseBase


class TestCharacters(RestApiTestCaseBase):

    def test_all_characters(self):
        body = self.fetch_json("characters")
        self.assertTrue(len(body) > 0)
        for character in body:
            self._check_character(character)

    def test_single_character(self):
        body = self.fetch_json("characters/1")
        self.assertEqual(len(body), 1)
        self.assertEqual(body[0]["char_id"], 1)
        self._check_character(body[0])

    def test_single_character_invalid_id(self):
        response = self.fetch("characters/invalid_id")
    ...
    def test_single_character_not_existing(self):
        ...
        body = self.fetch_json("characters/" + str(num + 1))
        self.assertEqual(len(body), 0)

    def test_character_by_category(self):
        body = self.fetch_json("characters/?category=Better+Call+Saul")
        self.assertTrue(len(body) > 0)
        for character in body:
            self._check_character(character)
    ...
    def test_character_by_category_limit(self):
        body1 = self.fetch_json("characters/?category=Better+Call+Saul&limit=2")
        self.assertTrue(len(body1) <= 2)
        body2 = self.fetch_json("characters/?category=Better+Call+Saul&limit=2&offset=2")
        self.assertNotEqual(body1, body2)

    def test_character_by_random(self):
        body = self.fetch_json("character/random")
    ...
    def test_character_limit_offset(self):
        ...
        count = 10
        while count > 0:
            body = self.fetch_json(f"characters?limit={step}&offset={offset}")
            offset += step
            count = len(body)
    ...
    def test_character_invalid_limit(self):
        step = -10
        body = self.fetch_json(f"characters?limit={step}")
        self.assertTrue(len(body) == 0)

    def test_character_by_name(self):
        body = self.fetch_json("characters/?name=Walter+White")
        self.assertTrue(len(body) == 1)
        self._check_character(body[0])
    ...
    def test_character_by_invalid_name(self):
        name = "unknown"
        body = self.fetch_json(f"characters/?name={name}")
        self.assertTrue(len(body) == 0)

    def test_character_by_part_name(self):
        body = self.fetch_json("characters/?name=Walter")
        self.assertTrue(len(body) > 0)
        for character in body:
            self._check_character(character)
